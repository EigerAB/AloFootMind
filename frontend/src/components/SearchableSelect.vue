<template>
  <div ref="containerRef" class="relative">
    ...
    >
      <span :class="displayLabel ? 'text-gray-300' : 'text-gray-500'">
    ...
    >
      ...
      <div class="max-h-48 overflow-y-auto">
        <button
          ...
          :class="isNullSelected ? 'bg-green-900/30 text-green-400' : 'text-gray-400'"
        >
          {{ nullLabel }}
        </button>
        <button
          ...
          :class="isSelected(item) ? 'bg-green-900/30 text-green-400' : ''"
        >
          {{ item[displayKey] }}
        </button>
...
const emit = defineEmits<{
  ...
  (e: 'select', value: Record<string, any> | null): void
}>()
...
const isNullSelected = computed(() => props.modelValue === null && props.nullLabel !== undefined)

const displayLabel = computed(() => {
  if (props.modelValue === null) return isNullSelected.value ? props.nullLabel : null
  return props.modelValue[props.displayKey] ?? null
})
...
function isSelected(item: Record<string, any>) {
  return props.modelValue !== null && props.modelValue[props.trackKey] === item[props.trackKey]
}
